...
void modbus_init()
{
    ...
    if(counter == 1)
    {
      ...
      value_voltage = *reinterpret_cast<float*>(data);
    }
    else if(counter == 2)
    { 
      //Paket Data 2 (Current)
      std::reverse(data, data + 2);  
      value_current = *reinterpret_cast<float*>(data);
    }
    else if(counter == 3)
    { 
      //Paket Data 3 (Power Active)
      std::reverse(data, data + 2);  
      value_power_active = *reinterpret_cast<float*>(data);
    }
    else if(counter == 4)
    {
      //Paket Data 4 (Temperature)
      std::reverse(data, data + 2);  
      value_temperature = *reinterpret_cast<float*>(data);
    }
  });
